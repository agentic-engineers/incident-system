...
def create_incident(payload: IncidentIn, session: Session = Depends(get_session)) -> Incident:
    incident = dedup.ingest_incident(session, payload.source, payload.title, payload.body)
    return incident


@app.get("/incidents")
def list_incidents(session: Session = Depends(get_session)) -> list[IncidentOut]:
...
def count_incidents(session: Session = Depends(get_session)) -> dict[str, int]:
...
def vm_start(payload: ConfirmRequest, session: Session = Depends(get_session)) -> dict[str, str]:
...
@app.get("/infra/dns")
def dns_get() -> dict[str, list[dict]]:
    try:
...
def dns_restore(
    payload: ConfirmRequest, session: Session = Depends(get_session)
) -> dict[str, list[str]]:
    if not payload.confirm:
        raise HTTPException(status_code=400, detail="mutacion: se requiere confirm=true")
    try:
